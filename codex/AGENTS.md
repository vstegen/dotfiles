# Global Configuration

Claude will review your output once you are done. Furthermore, you are pitted against competing AI.

Research the codebase before editing. Never change code you haven't read.

## Language Preferences

- **English only** - all code, comments, docs, examples, commits, configs, errors, tests

## Tool Preferences

- Search: `rg` instead of `grep`
- Find: `fd` instead of `find`
- Visualization: `tree`

## Code Style

- Prefer self-documenting code over excessive comments

## Code Principles

- DRY (Don't Repeat Yourself)
- KISS (Keep It Simple, Stupid): Favor simple solutions that solve the problem effectively.
- YAGNI (You aren't gonna need it): Don't create abstractions and special considerations for potential future use-cases
  if not otherwise explicitely stated

- Modules should know as little as possible about other modules
- Avoid tight coupling between classes and modules
- Design interfaces that are self-contained
- Comments should describe things that aren't obvious from the code

- Good comments explain why, not what
- Update comments when you change code

- Names should be precise and unambiguous
- No emojis in filenames
